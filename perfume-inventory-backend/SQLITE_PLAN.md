...
`create_all` only ever creates missing tables. It will not add a column or
change one on a table that already exists, so once you have data you need
migrations. Alembic is the standard tool. Everything below was run end to end
...
Replace the generated `migrations/env.py` with this. Two things matter: it
pulls the engine and metadata from your own code so there is one source of
truth, and it turns on batch mode.
...
# Import the app's engine and every module that defines a table, so that
# SQLModel.metadata knows about them before autogenerate runs.
...
def run_migrations_online() -> None:
    with engine.connect() as connection:
        context.configure(
            ...
            # behind the scenes instead. Without this, altering or dropping a
            # column fails.
...
**`render_as_batch=True` is mandatory on SQLite.** SQLite cannot `ALTER` most
things. Batch mode works around it by creating a new table, copying the rows
over, and swapping the names. Without this flag, any migration that alters or
drops a column fails.

One more edit. Autogenerated migrations refer to
`sqlmodel.sql.sqltypes.AutoString`, but the template does not import it. Add
the import to `migrations/script.py.mako` once:
...
Change `models.py`, then:

```bash
uv run alembic revision --autogenerate -m "add location column"
uv run alembic upgrade head
```

Always read the generated file in `migrations/versions/` before applying it.
...
### Adding a NOT NULL column to a table that has rows

This is the failure you will hit first. A model default is a Python default,
invisible to the database, so Alembic emits `nullable=False` with nothing to
backfill existing rows and the migration dies with:

```
sqlite3.IntegrityError: NOT NULL constraint failed: _alembic_tmp_perfume.location
```

Declare a database-level default in the model and autogenerate gets it right
with no hand editing:

```python
location: str = Field(
    default="Backroom",
    sa_column_kwargs={"server_default": "Backroom"},
)
```

Or edit the generated migration and add `server_default` to the column. Either
way the existing rows are backfilled and the data survives. Verified: a row
present before the migration kept its values and picked up the new default.

### If a batch migration fails partway

It leaves a scratch table behind, and the next attempt reports
`table _alembic_tmp_perfume already exists`. That message is a symptom, not the
cause. Drop the leftover, then fix the real error:
...
Leaving `create_db_and_tables()` in the lifespan hook means a fresh database
gets its tables from `create_all` with no `alembic_version` row. Alembic then
believes the database is empty and tries to create the tables again, which
fails with "table already exists". Remove the call from `main.py` and let
`alembic upgrade head` be the only thing that builds the schema.

For a database that already has the right tables but no version row, tell
Alembic where it stands instead of re-running the migration:
